...
- **用詞審慎保守原則**：除非研究文獻或臨床指引證據非常確切（例如被指引正式列為 Harm 級危害之明確醫療禁忌），否則文章中不可任意使用「禁止」、「禁用」等非保守詞彙，應一律使用「建議避免」、「不建議」或「建議優先考慮」等客觀審慎之保守臨床用語。
...
- **獨立主題叢集隔離**：新增專題時，若有系列導覽需求，應在 `build_static_articles.py` 中劃分獨立的主題叢集（Cluster），確保既有舊文章之上一篇／下一篇導覽鏈與內文 100% 保持乾淨零變更（Git 零 Diff），徹底杜絕雲端同步衝突檔（`[conflicted]`）。
