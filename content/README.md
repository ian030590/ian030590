...
- 保留推薦強度與不確定性；「可考慮」不宜改成「必須」，相關性不宜改成因果，無顯著差異不宜寫成等效，測驗進步不宜直接寫成生活功能恢復。呈現與問題相關的陰性或不一致結果。
...
- **核心理論依據（Theoretical Foundations）**：
...
- **禁止使用之不必要副詞與口語填絮詞（Prohibited Unnecessary Adverbs）**：
...
- **三大精煉與替換法則（Three Refinement & Substitution Rules）**：
...
### 嚴禁覆蓋既有舊文章準則（Zero Overwrite & Non-Destructive Addition）

- **零改動原則（Zero Overwrite）**：嚴禁修改、改寫、重構或覆蓋磁碟與 Git 上既有的舊文章。
- **全數接續新序號**：凡是根據新需求、新文獻（如教科書新章節）或新專題撰寫文章，一律必須使用目錄中未曾使用之全新序號撰寫獨立的新檔案（接續於當前最大序號之後，如 `029_...`），嚴禁插入舊文章序號區間。
- **獨立主題叢集隔離**：新增專題時，若有系列導覽需求，應在 `build_static_articles.py` 中劃分獨立的主題叢集（Cluster），確保既有舊文章之上一篇／下一篇導覽鏈與內文 100% 保持乾淨零變更（Git 零 Diff），避免雲端同步衝突檔（`[conflicted]`）。
...
| **應激** | **壓力** | Stress / Stress Response | 生理醫學／臨床神經復健 | 「應激」為中國大陸對 Stress 之譯名（如「應激反應」、「氧化應激」），台灣醫學、神經生理、精神心理學界與國家教育研究院學術名詞統一採用「壓力」（如「壓力反應」、「氧化壓力」、「代謝壓力」），文章中嚴禁使用「應激」，一律使用「壓力」。 |
...
- **禁止作業系統衝突字元與同步後綴**：嚴禁使用半形冒號 `:`、全形冒號 `：`、斜線 `/` `\`、半形問號 `?`、全形問號 `？`、引號 `"` `'`、星號 `*`、管道符 `|`、空白字元，以及雲端同步衝突標記（如 `[conflicted]`、`[conflicted 2]`）。文章標題中原本若有冒號，在檔名中轉換為底線 `_` 或直接連綴。
- **副檔名**：一律為 `.html`。
- **連結完整性保證**：檔案更名或新增時，必須確保與 `blog.html`、`js/articles-data.js` 及前後篇導覽之超連結絕對一致，零容忍 404 死連結。
...
- **嚴格按真實撰寫日期標註**：所有新發布、重構或重要修訂之文章，HTML 中繼標籤（`article:published_time`、`article:modified_time`）、JSON-LD 結構化資料（`datePublished`、`dateModified`）與內文標題欄 `<time datetime="...">` 必須填寫**文章實際撰寫完成當日的真實日期**（格式統一為 `YYYY-MM-DDT08:00:00+08:00`）。
...
6. **執行靜態建置與斷鏈檢驗**：
   ...
   ```
   建置腳本已內建自動死連結與衝突檔防禦機制，會自動檢驗 `blog.html`、`js/articles-data.js` 與文章內部連結是否 100% 存在且有效。確認終端顯示 `Validation passed` 且無任何 broken link。
