...
def update_file(path, new_url, new_cap):
    with open(path, 'r', encoding='utf-8') as f:
        html = f.read()

    clean_alt = re.sub(r'圖片來源.*$', '', new_cap).strip()

    # 1. Update og:image
    # <meta content="..." property="og:image"/> OR <meta property="og:image" content="..."/>
    html = re.sub(r'<meta[^>]+property=["\']og:image["\'][^>]*>', f'<meta content="{new_url}" property="og:image"/>', html)

    # 2. Update twitter:image
    html = re.sub(r'<meta[^>]+name=["\']twitter:image["\'][^>]*>', f'<meta content="{new_url}" name="twitter:image"/>', html)

    # 3. Update JSON-LD schema
    # In JSON-LD schema, replace the image url whether in list or string
    # We can match any URL with unsplash photo ID inside <script type="application/ld+json">
    def fix_schema(match):
        script_content = match.group(1)
        # replace any https://images.unsplash.com/photo-[^"'\s]+ with new_url
        updated_script = re.sub(r'https://images\.unsplash\.com/photo-[^"\'\s]+', new_url, script_content)
        return f'<script type="application/ld+json">{updated_script}</script>'

    html = re.sub(r'<script type="application/ld\+json">(.*?)</script>', fix_schema, html, flags=re.DOTALL)

    # 4. Update <figure class="article-featured-figure">
    def fix_figure(match):
        fig_inner = match.group(1)
        # replace <img ... class="article-featured-img" ... />
        fig_inner = re.sub(
            r'<img[^>]+class=["\']article-featured-img["\'][^>]*>',
            f'<img alt="{clean_alt}" class="article-featured-img" decoding="async" height="800" loading="lazy" src="{new_url}" width="1200"/>',
            fig_inner
        )
        # replace <figcaption ...>...</figcaption>
        fig_inner = re.sub(
            r'<figcaption[^>]*>.*?</figcaption>',
            f'<figcaption class="article-figcaption">{new_cap}</figcaption>',
            fig_inner,
            flags=re.DOTALL
        )
        return f'<figure class="article-featured-figure">{fig_inner}</figure>'

    html = re.sub(r'<figure class=["\']article-featured-figure["\']>(.*?)</figure>', fix_figure, html, flags=re.DOTALL)

    with open(path, 'w', encoding='utf-8') as f:
        f.write(html)
    print(f"Updated {path}")

for rel_path, (url, cap) in replacements.items():
    update_file(rel_path.replace('/', '\\'), url, cap)

print("\nAll 21 files updated cleanly!")
