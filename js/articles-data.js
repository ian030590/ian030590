// Generated static articles catalog (No Blogger dependencies)
window.__STATIC_ARTICLES__ = [
  {
    "id": "DigitalLearning_001",
    "title": "【系列總覽】不懂寫程式，也能做 AI 產品？非工程師的「AI 產品架構思維」12 講",
    "lead": "寫代碼的門檻正在崩塌，但系統架構與資安邊界的認知門檻卻前所未有地提高。專為產品經理、創業者、商務決策者打造的科技新聞級系統架構指南。",
    "imageUrl": "https://images.unsplash.com/photo-1522071820081-009f0129c71c?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-01-14",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/001_AI產品架構思維系列總覽_非工程師的12講導讀總綱.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "導讀總綱",
    "order": 1,
    "folder": "DigitalLearning",
    "filename": "001_AI產品架構思維系列總覽_非工程師的12講導讀總綱.html"
  },
  {
    "id": "DigitalLearning_002",
    "title": "破除全能迷思！用「聘請頂尖實習生」的心態理解大型語言模型",
    "lead": "做 AI 產品的第一步不是學寫代碼，而是卸下對大模型的不切實際幻想。搞懂底層的文字接龍與高維機率抽樣，才能在產品設計中劃出清晰的確定性邊界。",
    "imageUrl": "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-01-29",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/002_心智模型_破除全能迷思用聘請頂尖實習生的心態理解大型語言模型.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "基礎認知與軟體架構",
    "order": 2,
    "folder": "DigitalLearning",
    "filename": "002_心智模型_破除全能迷思用聘請頂尖實習生的心態理解大型語言模型.html"
  },
  {
    "id": "DigitalLearning_003",
    "title": "非工程師的軟體樂高課——前端、後端、API 與資料庫到底在幹嘛？",
    "lead": "別以為呼叫 API 就能直接上線！從米其林餐廳的協同運作看懂現代軟體四件套，避開導致信用卡被秒刷破產的致命架構漏洞。",
    "imageUrl": "https://images.unsplash.com/photo-1558494949-ef010cbdcc31?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-02-07",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/003_軟體骨架_非工程師的軟體樂高課前端後端API與資料庫到底在幹嘛.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "基礎認知與軟體架構",
    "order": 3,
    "folder": "DigitalLearning",
    "filename": "003_軟體骨架_非工程師的軟體樂高課前端後端API與資料庫到底在幹嘛.html"
  },
  {
    "id": "DigitalLearning_004",
    "title": "別再盲目摸索提示詞！從「隨機試錯」到「標準作業程序」",
    "lead": "在提示詞裡懇求或許諾小費無法建立商業壁壘。理解思維鏈（CoT）的推導機理，像連鎖餐飲制定 SOP 一樣鎖定輸入輸出資料契約。",
    "imageUrl": "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-02-22",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/004_提示工程_別再盲目摸索提示詞寫出工業級Prompt的標準作業程序.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "提示工程與知識檢索",
    "order": 4,
    "folder": "DigitalLearning",
    "filename": "004_提示工程_別再盲目摸索提示詞寫出工業級Prompt的標準作業程序.html"
  },
  {
    "id": "DigitalLearning_005",
    "title": "給 AI 一個外掛大腦！為什麼 RAG 是企業防胡說八道的必修課？",
    "lead": "別花冤枉錢微調大模型！從開卷考試哲學理解檢索增強生成（RAG），避開吞噬關鍵資訊的「長文本中間失憶症」。",
    "imageUrl": "https://images.unsplash.com/photo-1521587760476-6c12a4b040da?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-03-03",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/005_知識外掛_終結AI瞎掰RAG檢索增強生成如何打造專屬外掛大腦.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "提示工程與知識檢索",
    "order": 5,
    "folder": "DigitalLearning",
    "filename": "005_知識外掛_終結AI瞎掰RAG檢索增強生成如何打造專屬外掛大腦.html"
  },
  {
    "id": "DigitalLearning_006",
    "title": "會思考還會動手！自主 AI 智慧體是得力助手還是碎鈔機？",
    "lead": "當模型從純文本對話跨入工具調用（Tool Use），如何透過 ReAct 思考循環駕馭任務？非工程師必設的步數熔斷與人類簽核防線。",
    "imageUrl": "https://images.unsplash.com/photo-1485827404703-89b55fcc595e?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-03-18",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/006_智慧體行動_會思考還會動手自主AI智慧體是得力助手還是碎鈔機.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "自主智慧體與多Agent協同",
    "order": 6,
    "folder": "DigitalLearning",
    "filename": "006_智慧體行動_會思考還會動手自主AI智慧體是得力助手還是碎鈔機.html"
  },
  {
    "id": "DigitalLearning_007",
    "title": "一人開一間虛擬公司？多智慧體協同與 AI 界的「Type-C」接口（MCP）",
    "lead": "單打獨鬥的時代已經過去！理解多 Agent 的組織協同分工與死鎖防禦，掌握 2024 年底震撼開源界的 Anthropic MCP 標準化協定。",
    "imageUrl": "https://images.unsplash.com/photo-1544197150-b99a580bb7a8?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-03-27",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/007_協同協定_一人開一間虛擬公司多智慧體協同與AI界TypeC接口MCP.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "自主智慧體與多Agent協同",
    "order": 7,
    "folder": "DigitalLearning",
    "filename": "007_協同協定_一人開一間虛擬公司多智慧體協同與AI界TypeC接口MCP.html"
  },
  {
    "id": "DigitalLearning_008",
    "title": "「隨興編程」是效率革命還是維運災難？不懂代碼如何用 AI 開發工具不翻車？",
    "lead": "Cursor 與 v0 讓代碼生成快如閃電，但為何 80% 的專案最終陷入代碼腐化而爛尾？學會用「規格書思維」與 Git 時光機為專案買保險。",
    "imageUrl": "https://images.unsplash.com/photo-1555066931-4365d14bab8c?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-04-11",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/008_防翻車指南_隨興編程是效率革命還是維運災難不懂代碼如何用AI開發工具不翻車.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "工程防護與資訊安全",
    "order": 8,
    "folder": "DigitalLearning",
    "filename": "008_防翻車指南_隨興編程是效率革命還是維運災難不懂代碼如何用AI開發工具不翻車.html"
  },
  {
    "id": "DigitalLearning_009",
    "title": "AI 時代的駭客江湖！一句話就能套出你公司機密的「提示詞注入攻擊」",
    "lead": "當自然語言變成了可執行代碼，連小學生都能發動資安滲透。剖析 OWASP 榜首的直接與間接提示詞注入，非技術人必懂的三大防禦常識。",
    "imageUrl": "https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-04-20",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/009_資訊安全_AI時代的駭客江湖一句話就能套出公司機密的提示詞注入攻擊.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "工程防護與資訊安全",
    "order": 9,
    "folder": "DigitalLearning",
    "filename": "009_資訊安全_AI時代的駭客江湖一句話就能套出公司機密的提示詞注入攻擊.html"
  },
  {
    "id": "DigitalLearning_010",
    "title": "給 AI 戴上安全韁繩！如何打造不說髒話、不洩漏個資的「防護欄」",
    "lead": "快遞公司 AI 寫打油詩痛罵自家品牌？不想吃上 GDPR 巨額罰款或公關炎上，掌握三道安檢門禁與 PII 數據脫敏工程。",
    "imageUrl": "https://images.unsplash.com/photo-1563986768609-322da13575f3?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-05-05",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/010_安全護欄_給AI戴上安全韁繩如何打造不說髒話不洩漏個資的防護欄.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "工程防護與資訊安全",
    "order": 10,
    "folder": "DigitalLearning",
    "filename": "010_安全護欄_給AI戴上安全韁繩如何打造不說髒話不洩漏個資的防護欄.html"
  },
  {
    "id": "DigitalLearning_011",
    "title": "你的 AI 到底有多聰明？別再「看感覺」了，讓 AI 當裁判的科學評測法",
    "lead": "微調提示詞後只隨手抽測三題就宣稱優化成功？學會柏克萊大學 LLM-as-a-Judge 科學打分法，避開模型「自肥偏袒」的評測死角。",
    "imageUrl": "https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-05-14",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/011_量化評測_你的AI到底有多聰明別再看感覺讓AI當裁判的科學評測法.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "評測成本與長期維運",
    "order": 11,
    "folder": "DigitalLearning",
    "filename": "011_量化評測_你的AI到底有多聰明別再看感覺讓AI當裁判的科學評測法.html"
  },
  {
    "id": "DigitalLearning_012",
    "title": "每聊一句都在燒錢？AI 產品經理必看的省錢與提速秘密",
    "lead": "軟體吃到飽訂閱制的末日！理解 Token 運算邊際成本，掌握史丹佛 FrugalGPT 階梯路由、提示快取與打字串流體驗。",
    "imageUrl": "https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-05-29",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/012_成本與延遲_每聊一句都在燒錢破解AI產品的負毛利陷阱與延遲控制.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "評測成本與長期維運",
    "order": 12,
    "folder": "DigitalLearning",
    "filename": "012_成本與延遲_每聊一句都在燒錢破解AI產品的負毛利陷阱與延遲控制.html"
  },
  {
    "id": "DigitalLearning_013",
    "title": "發布不是結束！AI 產品上線後如何不生病、不擺爛的「長照指南」",
    "lead": "模型三個月後自己變笨了？從 Google 機器學習隱性技術債看穿 95% 的冰山下工程，掌握版本固定、雙供應商容災與負評資料飛輪。",
    "imageUrl": "https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "數位學習",
      "AI應用"
    ],
    "dateString": "2026-06-07",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/DigitalLearning/013_生命週期運維_發布不是結束AI產品上線後如何不生病不擺爛的長照指南.html",
    "sourceName": "數位學習",
    "category": "數位學習",
    "cluster": "非工程師的「AI 產品架構思維」12 講專題",
    "subCluster": "評測成本與長期維運",
    "order": 13,
    "folder": "DigitalLearning",
    "filename": "013_生命週期運維_發布不是結束AI產品上線後如何不生病不擺爛的長照指南.html"
  },
  {
    "id": "OccupationalTherapy_001",
    "title": "二十年中風復健典範轉移：從經驗主義到高強度神經重塑與急性期下床黃金律",
    "lead": "二十年前，中風復健常被視為『病況穩定後的被動休養』與經驗性神經發展手法；然而 2026 年美國心臟/中風協會（AHA/ASA）發布的最新指引標誌著劃時代的科學轉向：復健不再有『半年後定局』的終點，急性期過度激進的 24 小時內高強度動員被列為有害禁令，取而代之的是基於神經可塑性的終身連續性照護與高劑量任務訓練。",
    "imageUrl": "https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "動作復健"
    ],
    "dateString": "2026-01-05",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/001_二十年中風復健典範轉移_從經驗主義到高強度神經重塑與急性期下床黃金律.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風神經復健與全人照護",
    "subCluster": "急性期與動作功能重建",
    "order": 1,
    "folder": "OccupationalTherapy",
    "filename": "001_二十年中風復健典範轉移_從經驗主義到高強度神經重塑與急性期下床黃金律.html"
  },
  {
    "id": "OccupationalTherapy_002",
    "title": "上肢動作與精細手功能復健：任務導向訓練、局限誘發療法 CIMT 與 Bobath 技術的實證除魅",
    "lead": "上肢偏癱常導致極度挫折的『習得廢用』。在過去 20 年間，曾被廣泛運用的神經發展技術（Bobath）在 2026 最新指引中被正式除魅（評為對改善肌力與動作無效）；實證醫學全面確立了高重複、任務導向訓練與局限誘發動作療法（CIMT）的無可撼動核心，並引入了迷走神經刺激（VNS）等劃時代神經調控科技。",
    "imageUrl": "https://images.unsplash.com/photo-1584515979956-d9f6e5d09982?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "動作復健"
    ],
    "dateString": "2026-01-11",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/002_上肢動作與精細手功能復健_任務導向訓練、局限誘發療法_CIMT_與_Bobath_技術的實證除魅.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風神經復健與全人照護",
    "subCluster": "急性期與動作功能重建",
    "order": 2,
    "folder": "OccupationalTherapy",
    "filename": "002_上肢動作與精細手功能復健_任務導向訓練、局限誘發療法_CIMT_與_Bobath_技術的實證除魅.html"
  },
  {
    "id": "OccupationalTherapy_003",
    "title": "下肢步態與移動能力重建：地面任務行走、高強度間歇步態與機器人外骨骼的實證界線",
    "lead": "機器人外骨骼與減重懸吊跑步機曾被視為步態復健的終極未來；然而 2026 最新指引明確劃清界線：對於已有自主行走能力的中風患者，昂貴的機器人與低中強度跑步機不推薦優於真實地面訓練！步態重塑的核心重回高強度、挑戰平衡的地面任務跨步與雙下肢阻力訓練。",
    "imageUrl": "https://images.unsplash.com/photo-1576765608535-5f04d1e3f289?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "動作復健"
    ],
    "dateString": "2026-01-20",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/003_下肢步態與移動能力重建_地面任務行走、高強度間歇步態與機器人外骨骼的實證界線.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風神經復健與全人照護",
    "subCluster": "急性期與動作功能重建",
    "order": 3,
    "folder": "OccupationalTherapy",
    "filename": "003_下肢步態與移動能力重建_地面任務行走、高強度間歇步態與機器人外骨骼的實證界線.html"
  },
  {
    "id": "OccupationalTherapy_004",
    "title": "肌張力異常與痙攣的階層化管理：肉毒桿菌毒素注射、神經溶解與拉筋副木的科學定位",
    "lead": "中風後痙攣（Spasticity）並非單純靠拉筋就能解決的局部問題。2026 最新指引明確重構了痙攣治療路徑：長期佩戴手腕靜態副木與貼紮被列為無法預防痙攣的無效手段（COR 3: No Benefit）；肉毒桿菌毒素（BoNT）超音波/肌電導引注射確立為第一線標準，更重要的是——注射創造的 3 個月『可塑性開窗期』必須...",
    "imageUrl": "https://images.unsplash.com/photo-1584515933487-779824d29309?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "動作復健"
    ],
    "dateString": "2026-01-26",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/004_肌張力異常與痙攣的階層化管理_肉毒桿菌毒素注射、神經溶解與拉筋副木的科學定位.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風神經復健與全人照護",
    "subCluster": "急性期與動作功能重建",
    "order": 4,
    "folder": "OccupationalTherapy",
    "filename": "004_肌張力異常與痙攣的階層化管理_肉毒桿菌毒素注射、神經溶解與拉筋副木的科學定位.html"
  },
  {
    "id": "OccupationalTherapy_005",
    "title": "偏癱肩部疼痛與關節半脫位防護：肩袖力學保護、神經調控止痛與滑輪拉力的傷害警示",
    "lead": "偏癱肩痛（HSP）發生率高達 80%，更是阻礙手臂功能復原的頭號元兇。2026 最新 AHA/ASA 指引發布了極具警示意義的規範：過去常見的『懸空滑輪拉力運動』因極易扯裂肩袖，被列為危害等級明確禁用（COR 3: Harm）；取而代之的是早期功能性電刺激（NMES）保護關節囊、超音波導引神經阻斷與精準懸吊防護。",
    "imageUrl": "https://images.unsplash.com/photo-1532938911079-1b06ac7ceec7?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "動作復健"
    ],
    "dateString": "2026-02-04",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/005_偏癱肩部疼痛與關節半脫位防護_肩袖力學保護、神經調控止痛與滑輪拉力的傷害警示.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風神經復健與全人照護",
    "subCluster": "急性期與動作功能重建",
    "order": 5,
    "folder": "OccupationalTherapy",
    "filename": "005_偏癱肩部疼痛與關節半脫位防護_肩袖力學保護、神經調控止痛與滑輪拉力的傷害警示.html"
  },
  {
    "id": "OccupationalTherapy_006",
    "title": "大腦認知執行功能與空間忽略復健：代償策略訓練、稜鏡適應與眼球操迷思之終結",
    "lead": "中風後的認知衰退與單側空間忽略（Spatial Neglect），往往比肢體癱瘓更致命地摧毀患者的獨立生活能力。2026 最新 AHA/ASA 指引明確淘汰了無效的眼球轉動操與彩色濾光鏡；認知復健全面邁向『代償策略訓練』與『自適應電腦化訓練』，針對單側忽略則建立了稜鏡適應與主動視覺掃描之實證介入矩陣。",
    "imageUrl": "https://images.unsplash.com/photo-1559757175-5700dde675bc?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "認知復健",
      "視覺復健"
    ],
    "dateString": "2026-02-13",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/006_大腦認知執行功能與空間忽略復健_代償策略訓練、稜鏡適應與眼球操迷思之終結.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風神經復健與全人照護",
    "subCluster": "急性期與動作功能重建",
    "order": 6,
    "folder": "OccupationalTherapy",
    "filename": "006_大腦認知執行功能與空間忽略復健_代償策略訓練、稜鏡適應與眼球操迷思之終結.html"
  },
  {
    "id": "OccupationalTherapy_007",
    "title": "語言重構與運動性言語障礙：集約式失語症治療、旋律語調療法與非言語口部運動的迷思",
    "lead": "失去說話與理解能力是中風後最孤立無援的創傷。在過去 20 年間，語言復健經歷了一場徹底的科學清理：傳統無聲的吹口哨、鼓腮幫子等『非言語口部運動（NSOMEs）』被指引正式列為無效；高強度集約式語言治療（Intensive SLT）、旋律語調療法（MIT）與溝通夥伴培訓，確立為語言神經網絡重組的真正支柱。",
    "imageUrl": "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "認知復健",
      "動作復健"
    ],
    "dateString": "2026-02-19",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/007_語言重構與運動性言語障礙_集約式失語症治療、旋律語調療法與非言語口部運動的迷思.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風神經復健與全人照護",
    "subCluster": "急性期與動作功能重建",
    "order": 7,
    "folder": "OccupationalTherapy",
    "filename": "007_語言重構與運動性言語障礙_集約式失語症治療、旋律語調療法與非言語口部運動的迷思.html"
  },
  {
    "id": "OccupationalTherapy_008",
    "title": "吞嚥障礙全流程管理與營養支持：進食前強制標準化篩檢、客觀儀器評估與管灌造瘻決策",
    "lead": "吸入性肺炎是中風急性期與亞急性期奪命的第一元兇。2026 最新指引建立前所未有的嚴格防線：任何口服飲食、喝水或服藥前，必須在 24 小時內完成標準化吞嚥篩檢；疑似吞嚥異常者，客觀儀器檢查（VFSS/FEES）全面確立為優於床邊推論的最高準則；長期無安全進食希望者，經皮胃造瘻（PEG）明確優於長期插置鼻胃管。",
    "imageUrl": "https://images.unsplash.com/photo-1505751172876-fa1923c5c528?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "動作復健"
    ],
    "dateString": "2026-02-28",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/008_吞嚥障礙全流程管理與營養支持_進食前強制標準化篩檢、客觀儀器評估與管灌造瘻決策.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風神經復健與全人照護",
    "subCluster": "急性期與動作功能重建",
    "order": 8,
    "folder": "OccupationalTherapy",
    "filename": "008_吞嚥障礙全流程管理與營養支持_進食前強制標準化篩檢、客觀儀器評估與管灌造瘻決策.html"
  },
  {
    "id": "OccupationalTherapy_009",
    "title": "隱形復健殺手：中風後憂鬱與焦慮的縱向篩檢、藥物合併心理治療及睡眠疲憊管理",
    "lead": "中風後憂鬱（PSD）與病態性疲憊（Poststroke Fatigue）發生率高達三分之一，是阻礙神經重塑成敗的隱形巨獸。2026 最新指引正式將心理健康與睡眠疲憊設立獨立章節：確立縱向終身篩檢憂鬱與焦慮；更破除了十年前的小型試驗迷思，明確禁止常規開立抗憂鬱劑百憂解（Fluoxetine）來治療中風後疲憊或促進動作修復...",
    "imageUrl": "https://images.unsplash.com/photo-1516585427167-9f4af9627e6c?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "認知復健"
    ],
    "dateString": "2026-03-09",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/009_隱形復健殺手_中風後憂鬱與焦慮的縱向篩檢、藥物合併心理治療及睡眠疲憊管理.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風神經復健與全人照護",
    "subCluster": "急性期與動作功能重建",
    "order": 9,
    "folder": "OccupationalTherapy",
    "filename": "009_隱形復健殺手_中風後憂鬱與焦慮的縱向篩檢、藥物合併心理治療及睡眠疲憊管理.html"
  },
  {
    "id": "OccupationalTherapy_010",
    "title": "全人生活重構與社會回歸：社區轉銜、照護者賦能、駕駛工作評估與遠距復健未來",
    "lead": "中風復健的終極成就，絕不是『在病房走廊走完十公尺』，而是重新做回一個有尊嚴的社會人。2026 最新指引將世界衛生組織 ICF 的『社會參與（Participation）』推向最高殿堂：照護者賦能與喘息支援正式升格為 COR 1 最高等級推薦；重返駕駛與職場建立了嚴格的多維度客觀評估標準；遠距復健則打破城鄉壁壘，實現長效...",
    "imageUrl": "https://images.unsplash.com/photo-1581579438747-1dc8d17bbce4?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "動作復健",
      "認知復健"
    ],
    "dateString": "2026-03-15",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/010_全人生活重構與社會回歸_社區轉銜、照護者賦能、駕駛工作評估與遠距復健未來.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風神經復健與全人照護",
    "subCluster": "急性期與動作功能重建",
    "order": 10,
    "folder": "OccupationalTherapy",
    "filename": "010_全人生活重構與社會回歸_社區轉銜、照護者賦能、駕駛工作評估與遠距復健未來.html"
  },
  {
    "id": "OccupationalTherapy_011",
    "title": "中風後視覺障礙全景透視：從急性期強制篩檢到三大介入流派（復原、代償、替代）的實證藍圖",
    "lead": "高達 60% 的中風倖存者伴隨不同程度的視覺障礙，卻常因肢體癱瘓或失語症而被嚴重忽略。2026 最新 AHA/ASA 指引明確規範：中風急性期必須落實標準化全方位視覺篩檢；更劃時代地將視覺復健確立為三大互補介入範式——復原性（Restitutive）、代償性（Compensatory）與替代性（Substitutive...",
    "imageUrl": "https://images.unsplash.com/photo-1579684385127-1ef15d508118?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "視覺復健"
    ],
    "dateString": "2026-03-24",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/011_中風後視覺障礙全景透視_從急性期強制篩檢到三大介入流派的實證藍圖.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風後神經視覺復健",
    "subCluster": "神經視覺功能重建與代償",
    "order": 11,
    "folder": "OccupationalTherapy",
    "filename": "011_中風後視覺障礙全景透視_從急性期強制篩檢到三大介入流派的實證藍圖.html"
  },
  {
    "id": "OccupationalTherapy_012",
    "title": "眼球動作控制與內聚力不足復健：從 Pencil Push-Ups 到日常生活任務導向訓練的實證指引",
    "lead": "看近物時雙眼無法向內對焦、字體模糊分裂成兩個影子——這是中風後常見卻極易被誤診的「內聚力不足（Convergence Insufficiency）」。在 2026 AHA/ASA 最新指引中，眼球運動訓練在內聚力不足項目榮獲最高榮譽 COR 1 (LOE A) 強烈推薦！結合代償性任務導向訓練更獲 COR 2a 推薦能...",
    "imageUrl": "https://images.unsplash.com/photo-1588776814546-1ffcf47267a5?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "視覺復健",
      "動作復健"
    ],
    "dateString": "2026-04-02",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/012_眼球動作控制與內聚力不足復健_從PencilPushUps到日常生活任務導向訓練的實證指引.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風後神經視覺復健",
    "subCluster": "神經視覺功能重建與代償",
    "order": 12,
    "folder": "OccupationalTherapy",
    "filename": "012_眼球動作控制與內聚力不足復健_從PencilPushUps到日常生活任務導向訓練的實證指引.html"
  },
  {
    "id": "OccupationalTherapy_013",
    "title": "同向偏盲與單側空間忽略的臨床鑑別：感覺輸入缺損與注意力網絡崩解之診斷與處方分野",
    "lead": "中風患者經常碰撞左側門框、吃飯漏掉左半邊菜餚——這究竟是「同向偏盲（Homonymous Hemianopia）」還是「單側空間忽略（Spatial Neglect）」？兩者在臨床表現上極為相似，但神經病理機制截然不同：前者是「眼睛到大腦的視覺線路被切斷」，後者則是「大腦注意力空間地圖崩解」。若將忽略誤當偏盲，復健將完...",
    "imageUrl": "https://images.unsplash.com/photo-1576671081837-49000212a370?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "視覺復健",
      "認知復健"
    ],
    "dateString": "2026-04-08",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/013_同向偏盲與單側空間忽略的臨床鑑別_感覺輸入缺損與注意力網絡崩解之診斷與處方分野.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風後神經視覺復健",
    "subCluster": "神經視覺功能重建與代償",
    "order": 13,
    "folder": "OccupationalTherapy",
    "filename": "013_同向偏盲與單側空間忽略的臨床鑑別_感覺輸入缺損與注意力網絡崩解之診斷與處方分野.html"
  },
  {
    "id": "OccupationalTherapy_014",
    "title": "視野缺損的光學替代療法：Fresnel 稜鏡與 Peli 稜鏡在同向偏盲中的光學校正與實證效果",
    "lead": "對於同向偏盲導致的周邊視野完全喪失，光學稜鏡（Optical Prisms）是現代神經復健最具實證支持的替代性輔具。2026 AHA/ASA 指引正式給予稜鏡在補償視野缺損 COR 2a (LOE B-R) 的高度肯定。本文深入剖析傳統全貼片 Fresnel 稜鏡與劃時代的 Peli 邊緣橫向稜鏡的光學力學差異、臨床適...",
    "imageUrl": "https://images.unsplash.com/photo-1513542789411-b6a5d4f31634?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "視覺復健"
    ],
    "dateString": "2026-04-17",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/014_視野缺損的光學替代療法_Fresnel稜鏡與Peli稜鏡在同向偏盲中的光學校正與實證效果.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風後神經視覺復健",
    "subCluster": "神經視覺功能重建與代償",
    "order": 14,
    "folder": "OccupationalTherapy",
    "filename": "014_視野缺損的光學替代療法_Fresnel稜鏡與Peli稜鏡在同向偏盲中的光學校正與實證效果.html"
  },
  {
    "id": "OccupationalTherapy_015",
    "title": "代償性視覺掃描訓練（VST）：提升視野缺損生活品質的黃金處方與「無法逆轉視野」的科學真相",
    "lead": "中風視野缺損患者最渴望聽到的承諾是「我的視野能不能長回來？」然而實證醫學必須給予誠實而強大的回答：2026 AHA/ASA 指引明確指出，代償性視覺掃描訓練（VST）能顯著提高視覺相關生活品質（COR 2a），但 Cochrane 大型回顧證實其「完全無法縮小客觀視野缺損」！復健的核心在於建立高效率的快速跳視（Sacc...",
    "imageUrl": "https://images.unsplash.com/photo-1449965408869-eaa3f722e40d?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "視覺復健",
      "認知復健"
    ],
    "dateString": "2026-04-26",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/015_代償性視覺掃描訓練_提升視野缺損生活品質的黃金處方與無法逆轉視野的科學真相.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風後神經視覺復健",
    "subCluster": "神經視覺功能重建與代償",
    "order": 15,
    "folder": "OccupationalTherapy",
    "filename": "015_代償性視覺掃描訓練_提升視野缺損生活品質的黃金處方與無法逆轉視野的科學真相.html"
  },
  {
    "id": "OccupationalTherapy_016",
    "title": "電腦化視覺復原訓練（VRT）：神經可塑性邊界探索、爭議對比與非侵入性腦刺激的前沿交鋒",
    "lead": "視野壞死真的能逆轉嗎？電腦化視覺復原訓練（VRT）與視覺辨別訓練（VDT）是神經學界爭議最烈、亦最具吸引力的領域。2026 AHA/ASA 指引將電腦化視覺復原訓練列為 COR 2b (LOE B-R) 推薦，肯定其在部分個案中擴展視野與改善生活品質的潛能，但同時嚴正指出學界對其「眼球漂移偽影」的質疑，並剖析了結合非侵...",
    "imageUrl": "https://images.unsplash.com/photo-1530497610245-94d3c16cda28?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "視覺復健"
    ],
    "dateString": "2026-05-02",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/016_電腦化視覺復原訓練_神經可塑性邊界探索爭議對比與非侵入性腦刺激的前沿交鋒.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風後神經視覺復健",
    "subCluster": "神經視覺功能重建與代償",
    "order": 16,
    "folder": "OccupationalTherapy",
    "filename": "016_電腦化視覺復原訓練_神經可塑性邊界探索爭議對比與非侵入性腦刺激的前沿交鋒.html"
  },
  {
    "id": "OccupationalTherapy_017",
    "title": "超越純視覺：視聽雙模態多感官空間探索訓練（Audio-Visual Training）的跨腦區神經協同機制",
    "lead": "長久以來，視覺復健被狹隘地侷限在「眼睛看眼睛的」。然而 2026 最新 AHA/ASA 指引投下了震撼彈：在所有視覺空間與知覺障礙介入中，唯一獲得最高榮譽 COR 1 (LOE B-R) 強烈推薦的，正是「視聽雙模態多感官空間探索訓練（Audio-Visual Training）」！多感官協同如何繞過受損枕葉、活化中腦...",
    "imageUrl": "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "視覺復健",
      "認知復健"
    ],
    "dateString": "2026-05-11",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/017_超越純視覺_視聽雙模態多感官空間探索訓練的跨腦區神經協同機制.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風後神經視覺復健",
    "subCluster": "神經視覺功能重建與代償",
    "order": 17,
    "folder": "OccupationalTherapy",
    "filename": "017_超越純視覺_視聽雙模態多感官空間探索訓練的跨腦區神經協同機制.html"
  },
  {
    "id": "OccupationalTherapy_018",
    "title": "行為視光學除魅：指引為何嚴格禁止無效眼球轉動操、彩色濾光鏡與偽科學鏡片（COR 3 禁令）",
    "lead": "轉轉眼球、畫米字操就能治好中風看不見？戴上昂貴的彩色濾光鏡片就能打通大腦知覺神經？在 2026 年最新 AHA/ASA 中風復健指引中，專家委員會發布了重磅實證禁令：正式將機械式眼球操、特殊鏡片與彩色濾光鏡列為「無效益等級明確不推薦（COR 3: No Benefit）」！本文揭露商業偽科學的話術陷阱，捍衛中風病患的黃...",
    "imageUrl": "https://images.unsplash.com/photo-1511499767150-a48a237f0083?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "視覺復健"
    ],
    "dateString": "2026-05-20",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/018_行為視光學除魅_指引為何嚴格禁止無效眼球轉動操彩色濾光鏡與偽科學鏡片.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風後神經視覺復健",
    "subCluster": "神經視覺功能重建與代償",
    "order": 18,
    "folder": "OccupationalTherapy",
    "filename": "018_行為視光學除魅_指引為何嚴格禁止無效眼球轉動操彩色濾光鏡與偽科學鏡片.html"
  },
  {
    "id": "OccupationalTherapy_019",
    "title": "偏盲性閱讀障礙（Hemianopic Alexia）：眼球快速跳躍訓練、文字版面調適與輔助科技實戰",
    "lead": "中風後「每個字都認得，但一行字就是讀不完、讀不懂」？高達 80% 的同向偏盲患者伴隨極具破壞性的「偏盲性閱讀障礙（Hemianopic Alexia）」。右側偏盲摧毀了中央凹旁預視視窗，左側偏盲則讓換行回跳陷入迷宮。2026 最新 AHA/ASA 指引明確將閱讀專用代償訓練列為 COR 2a 實證推薦！本文解析眼動儀跳...",
    "imageUrl": "https://images.unsplash.com/photo-1457369804613-52c61a468e7d?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "視覺復健",
      "認知復健"
    ],
    "dateString": "2026-05-26",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/019_偏盲性閱讀障礙_眼球快速跳躍訓練文字版面調適與輔助科技實戰.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風後神經視覺復健",
    "subCluster": "神經視覺功能重建與代償",
    "order": 19,
    "folder": "OccupationalTherapy",
    "filename": "019_偏盲性閱讀障礙_眼球快速跳躍訓練文字版面調適與輔助科技實戰.html"
  },
  {
    "id": "OccupationalTherapy_020",
    "title": "沉浸式虛擬實境（VR）復健、居家防跌環境重構與重返駕駛的嚴格多維度評估",
    "lead": "視覺復健的終極考驗在於真實世界——走出醫院，如何在家中不被障礙物絆倒骨折？中風後能否重新握起方向盤合法開車？2026 最新 AHA/ASA 指引將沉浸式虛擬實境（VR）列入視覺知覺介入（COR 2b），更對重返駕駛（Return to Driving）的法定視覺門檻與多維評估建立了嚴格標準。本文完整解構防跌環境改造與駕...",
    "imageUrl": "https://images.unsplash.com/photo-1592478411213-6153e4ebc07d?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "中風復健",
      "動作復健",
      "視覺復健",
      "認知復健"
    ],
    "dateString": "2026-06-04",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/OccupationalTherapy/020_沉浸式虛擬實境VR復健_居家防跌環境重構與重返駕駛的嚴格多維度評估.html",
    "sourceName": "中風復健",
    "category": "中風復健",
    "cluster": "中風後神經視覺復健",
    "subCluster": "神經視覺功能重建與代償",
    "order": 20,
    "folder": "OccupationalTherapy",
    "filename": "020_沉浸式虛擬實境VR復健_居家防跌環境重構與重返駕駛的嚴格多維度評估.html"
  },
  {
    "id": "VisualTherapy_001",
    "title": "眼科照護全光譜轉移：從疾病終點治療到 2023 AAO 視覺復健指引核心架構與三級照護模型",
    "lead": "長期以來，許多臨床醫師與患者誤以為視覺復健僅適用於重度視障或法定盲患者。然而，美國眼科學會（AAO）於 2023 年發布的《Vision Rehabilitation Preferred Practice Pattern®》正式宣告重大典範轉移：即使是輕度至中度視力下降，亦會顯著摧毀患者的閱讀能力、心理健康、社交參與並...",
    "imageUrl": "https://images.unsplash.com/photo-1581594693702-fbdc51b2763b?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健"
    ],
    "dateString": "2026-01-08",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/001_眼科照護全光譜轉移_從疾病終點治療到2023AAO視覺復健指引核心架構與三級照護模型.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "低視能臨床評估與光學處方科學",
    "subCluster": "功能評估與輔具處方",
    "order": 1,
    "folder": "VisualTherapy",
    "filename": "001_眼科照護全光譜轉移_從疾病終點治療到2023AAO視覺復健指引核心架構與三級照護模型.html"
  },
  {
    "id": "VisualTherapy_002",
    "title": "突破斯內倫視力表迷思：低視能精準屈光、ETDRS 與 Bailey-Lovie 遠近視力標準化評估",
    "lead": "傳統 Snellen 視力表在低視能臨床中存在致命缺陷：視標字數不均、間距比例不一致、缺少更低視能梯次，且無法進行精準統計運算。2023 AAO 視覺復健指引明確要求：低視能評估應常態化採用 Bailey-Lovie 或 ETDRS 等 logMAR 對數幾何級數視力表；評估嚴重視損時應主動縮短檢驗距離；屈光檢查必須採...",
    "imageUrl": "https://images.unsplash.com/photo-1591076482161-42ce6da69f67?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健"
    ],
    "dateString": "2026-01-17",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/002_突破斯內倫視力表迷思_低視能精準屈光、ETDRS與BaileyLovie遠近視力標準化評估.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "低視能臨床評估與光學處方科學",
    "subCluster": "功能評估與輔具處方",
    "order": 2,
    "folder": "VisualTherapy",
    "filename": "002_突破斯內倫視力表迷思_低視能精準屈光、ETDRS與BaileyLovie遠近視力標準化評估.html"
  },
  {
    "id": "VisualTherapy_003",
    "title": "真實世界的視覺之錨：對比敏感度在日常生活功能與跌倒風險評估之關鍵角色",
    "lead": "許多黃斑部病變、青光眼或白內障術後患者在診間測得 20/25 的優異高對比視力，日常生活卻寸步難行、頻頻踏空跌倒、無法辨識親友臉孔。2023 AAO 視覺復健指引明確強調：中心高對比視力僅反映黃斑部極小焦點能力，對比敏感度（Contrast Sensitivity）才是決定步行安全、階梯辨識、閱讀流暢度與臉部辨識的核心...",
    "imageUrl": "https://images.unsplash.com/photo-1574258495973-f010dfbb5371?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健",
      "動作復健"
    ],
    "dateString": "2026-01-23",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/003_真實世界的視覺之錨_對比敏感度在日常生活功能與跌倒風險評估之關鍵角色.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "低視能臨床評估與光學處方科學",
    "subCluster": "功能評估與輔具處方",
    "order": 3,
    "folder": "VisualTherapy",
    "filename": "003_真實世界的視覺之錨_對比敏感度在日常生活功能與跌倒風險評估之關鍵角色.html"
  },
  {
    "id": "VisualTherapy_004",
    "title": "凝視重生的導航技術：中心暗點微視野檢查與優選視網膜位點（PRL）臨床定位",
    "lead": "黃斑部病變（如濕性 AMD 地圖狀萎縮、斯特格氏病）摧毀中央凹中心凹視覺後，患者視野中央會形成破壞性的絕對或相對暗點（Scotoma）。2023 AAO 指引指出：傳統 Amsler 方格表無法客觀定量暗點，更無法評估注視穩定度。現代視覺復健全面引進「微視野檢查儀（Microperimetry）」，能在紅外線活體眼底圖...",
    "imageUrl": "https://images.unsplash.com/photo-1581594693702-fbdc51b2763b?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健"
    ],
    "dateString": "2026-02-01",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/004_凝視重生的導航技術_中心暗點微視野檢查與優選視網膜位點PRL臨床定位.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "低視能臨床評估與光學處方科學",
    "subCluster": "功能評估與輔具處方",
    "order": 4,
    "folder": "VisualTherapy",
    "filename": "004_凝視重生的導航技術_中心暗點微視野檢查與優選視網膜位點PRL臨床定位.html"
  },
  {
    "id": "VisualTherapy_005",
    "title": "偏心注視與定眼閱讀法：黃斑部病變之視網膜功能重組訓練與生物回饋實證真相",
    "lead": "當中央凹發生不可逆萎縮，患者本能地試圖「直視」目標只會導致影像消失在中央暗點中。2023 AAO 視覺復健指引深度探討了兩大核心技巧：偏心注視（Eccentric Viewing, EV）訓練與定眼閱讀法（Steady Eye Strategy）。透過將目光刻意偏離目標，將影像精準投射於殘存健全的視網膜位點（PRL）。...",
    "imageUrl": "https://images.unsplash.com/photo-1544717305-2782549b5136?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健",
      "認知復健"
    ],
    "dateString": "2026-02-10",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/005_偏心注視與定眼閱讀法_黃斑部病變之視網膜功能重組訓練與生物回饋實證真相.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "低視能臨床評估與光學處方科學",
    "subCluster": "功能評估與輔具處方",
    "order": 5,
    "folder": "VisualTherapy",
    "filename": "005_偏心注視與定眼閱讀法_黃斑部病變之視網膜功能重組訓練與生物回饋實證真相.html"
  },
  {
    "id": "VisualTherapy_006",
    "title": "近用光學輔具之處方科學：高屈光度正透鏡、立式放大鏡與手持放大鏡的幾何光學與任務適配",
    "lead": "光學輔具絕非隨意購買高倍率放大鏡即可奏效。2023 AAO 視覺復健指引明確闡明了光學放大的物理學法則：等效觀看距離（EVD）是所有近用放大的共通本質。高加光眼鏡（High-add microscopes）提供最大的視野（FOV），但代價是極短的工作距離與眼球過度內聚負擔，需配合基底朝內稜鏡（Base-in prism...",
    "imageUrl": "https://images.unsplash.com/photo-1563178406-4cdc2923acbc?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健"
    ],
    "dateString": "2026-02-16",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/006_近用光學輔具之處方科學_高屈光度正透鏡、立式放大鏡與手持放大鏡幾何光學與任務適配.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "低視能臨床評估與光學處方科學",
    "subCluster": "功能評估與輔具處方",
    "order": 6,
    "folder": "VisualTherapy",
    "filename": "006_近用光學輔具之處方科學_高屈光度正透鏡、立式放大鏡與手持放大鏡幾何光學與任務適配.html"
  },
  {
    "id": "VisualTherapy_007",
    "title": "電子與數位放大科技革命：桌上型擴視機（CCTV）、可攜式電子擴視機與高對比色彩模式實證優勢",
    "lead": "當視力下降至 20/200 甚至更差時，傳統光學放大鏡因視差扭曲與極短工作距離往往瀕臨失效。2023 AAO 視覺復健指引高度肯定電子擴視機（Electronic Video Magnifiers / CCTV）的劃時代臨床價值。透過高畫質相機與數位影像處理，電子擴視機能實現獨立於焦距的 2 至 60 倍無級縮放、大景...",
    "imageUrl": "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健"
    ],
    "dateString": "2026-02-25",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/007_電子與數位放大科技革命_桌上型擴視機CCTV、可攜式電子擴視機與高對比色彩模式實證優勢.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "低視能臨床評估與光學處方科學",
    "subCluster": "功能評估與輔具處方",
    "order": 7,
    "folder": "VisualTherapy",
    "filename": "007_電子與數位放大科技革命_桌上型擴視機CCTV、可攜式電子擴視機與高對比色彩模式實證優勢.html"
  },
  {
    "id": "VisualTherapy_008",
    "title": "行動智慧時代無障礙轉型：智慧型手機、平板電腦與 AI 視覺辨識在低視能日常代償之整合應用",
    "lead": "傳統專用低視能輔具價格昂貴、體積笨重且常帶有顯眼的「殘疾標籤（Stigma）」。2023 AAO 視覺復健指引迎來了劃時代的科技觀點：智慧型手機（iOS / Android）與平板電腦已普及成為視障族群最核心、最實用的輔助生活平台。從作業系統原生內建的動態字級、放大鏡、對比濾鏡、螢幕朗讀（VoiceOver/TalkB...",
    "imageUrl": "https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健",
      "AI應用"
    ],
    "dateString": "2026-03-06",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/008_行動智慧時代無障礙轉型_智慧型手機、平板電腦與AI視覺辨識在低視能日常代償之整合應用.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "低視能臨床評估與光學處方科學",
    "subCluster": "功能評估與輔具處方",
    "order": 8,
    "folder": "VisualTherapy",
    "filename": "008_行動智慧時代無障礙轉型_智慧型手機、平板電腦與AI視覺辨識在低視能日常代償之整合應用.html"
  },
  {
    "id": "VisualTherapy_009",
    "title": "文字解碼能力之客觀度量：MnREAD 閱讀功能評估尺度臨床應用與閱讀處方精準化",
    "lead": "單個孤立字母辨識（如 ETDRS）無法代表日常連續閱讀能力！真實閱讀涉及字詞整合、語意預測、眼球跳視與周邊文字跨度。2023 AAO 視覺復健指引推崇 Minnesota Low-Vision Reading Test（MnREAD）作為閱讀功能標準化量測工具。MnREAD 能精確解構三大臨床指標：閱讀視力（Readi...",
    "imageUrl": "https://images.unsplash.com/photo-1455390582262-044cdead277a?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健",
      "認知復健"
    ],
    "dateString": "2026-03-12",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/009_文字解碼能力之客觀度量_MnREAD閱讀功能評估尺度臨床應用與閱讀處方精準化.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "低視能臨床評估與光學處方科學",
    "subCluster": "功能評估與輔具處方",
    "order": 9,
    "folder": "VisualTherapy",
    "filename": "009_文字解碼能力之客觀度量_MnREAD閱讀功能評估尺度臨床應用與閱讀處方精準化.html"
  },
  {
    "id": "VisualTherapy_010",
    "title": "照亮暗處的處方藝術：低視能照明工程學（Lux、色溫、防眩光）與物理環境光學優化實務",
    "lead": "老化眼球角膜內皮細胞衰退、晶狀體發黃硬化以及視網膜感光細胞數量銳減，使得低視能長者抵達視網膜的光通量僅剩年輕時的三分之一甚至十分之一。2023 AAO 視覺復健指引將「照明評估與環境優化」列為最具成本效益的首要非光學處方。指引強調：單純調亮天花板大燈是危險的錯誤，低視能照明必須講究「照度（Lux）精準計量」、「色溫（K...",
    "imageUrl": "https://images.unsplash.com/photo-1473496169904-658ba7c44d8a?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健"
    ],
    "dateString": "2026-03-21",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/010_照亮暗處的處方藝術_低視能照明工程學Lux、色溫、防眩光與物理環境光學優化實務.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "環境人因工程、安全自理與防跌防護",
    "subCluster": "環境改造與居家自理",
    "order": 10,
    "folder": "VisualTherapy",
    "filename": "010_照亮暗處的處方藝術_低視能照明工程學Lux、色溫、防眩光與物理環境光學優化實務.html"
  },
  {
    "id": "VisualTherapy_011",
    "title": "濾光鏡片與眩光管理：吸收型濾光鏡、偏光鏡片在畏光控制與對比增強中臨床選擇",
    "lead": "眩光（Glare）絕不僅是「刺眼」這麼簡單。在伴隨角膜混濁、白內障、無虹膜或黃斑部萎縮的患者中，光線在眼球屈光介質中產生劇烈散射，形成徹底掩蓋影像的「失能性眩光（Disability Glare）」。2023 AAO 視覺復健指引深度剖析了醫療級吸收型濾光鏡片（Corning CPF, NoIR 等）之臨床價值：藉由特...",
    "imageUrl": "https://images.unsplash.com/photo-1511499767150-a48a237f0083?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健"
    ],
    "dateString": "2026-03-30",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/011_濾光鏡片與眩光管理_吸收型濾光鏡、偏光鏡片在畏光控制與對比增強中臨床選擇.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "環境人因工程、安全自理與防跌防護",
    "subCluster": "環境改造與居家自理",
    "order": 11,
    "folder": "VisualTherapy",
    "filename": "011_濾光鏡片與眩光管理_吸收型濾光鏡、偏光鏡片在畏光控制與對比增強中臨床選擇.html"
  },
  {
    "id": "VisualTherapy_012",
    "title": "居家生活自理（ADL/IADL）重構：高對比改造、觸覺標記與多感官代償之環境人因工學",
    "lead": "對於視力嚴重受損的長者而言，原本熟悉的家可能瞬間淪為危機四伏的戰場：白瓷盤裝白飯看不見邊緣、瓦斯爐旋鈕調錯火候、倒開水頻頻溢流燙傷、在樓梯踏空摔下。2023 AAO 視覺復健指引（特別在 Appendix 6 中）詳細規範了職能治療師（Occupational Therapist）在日常生活活動（ADL/IADL）中扮...",
    "imageUrl": "https://images.unsplash.com/photo-1556911220-e15b29be8c8f?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健",
      "動作復健"
    ],
    "dateString": "2026-04-05",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/012_居家生活自理重構_高對比改造、觸覺標記與多感官代償之環境人因工學.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "環境人因工程、安全自理與防跌防護",
    "subCluster": "環境改造與居家自理",
    "order": 12,
    "folder": "VisualTherapy",
    "filename": "012_居家生活自理重構_高對比改造、觸覺標記與多感官代償之環境人因工學.html"
  },
  {
    "id": "VisualTherapy_013",
    "title": "低視能長者的防跌防護網：白手杖定向行動（O&M）訓練、居家動線排查與跨專業防跌介入",
    "lead": "跌倒是摧毀視障長者生命尊嚴與加速死亡的頭號殺手。2023 AAO 視覺復健指引引用震撼流行病學數據指出：視力下降長者的跌倒風險增加近 2 倍，造成嚴重髖部骨折的風險更飆升 4 倍以上！傳統觀念常誤以為白手杖是「全盲專用」，使得長者因面子問題抗拒使用。指引大力倡導認證定向行動專家（COMS）與職能/物理治療師的協同介入：...",
    "imageUrl": "https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健",
      "動作復健"
    ],
    "dateString": "2026-04-14",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/013_低視能長者防跌防護網_白手杖定向行動訓練、居家動線排查與跨專業防跌介入.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "環境人因工程、安全自理與防跌防護",
    "subCluster": "環境改造與居家自理",
    "order": 13,
    "folder": "VisualTherapy",
    "filename": "013_低視能長者防跌防護網_白手杖定向行動訓練、居家動線排查與跨專業防跌介入.html"
  },
  {
    "id": "VisualTherapy_014",
    "title": "隱形用藥風暴之防線：視力障礙患者安全給藥管理、智慧藥盒與語音標籤防錯機制",
    "lead": "低視能長者往往罹患多重慢性疾病（心臟病、高血壓、糖尿病、青光眼），平均每日需服用 5 至 9 種不同藥物。然而，常規藥袋上字體極小（常僅 6-8pt）、透明膠囊外觀相似、眼藥水瓶身無從區分，導致用藥錯誤率、重複服藥與漏服率高達 30% 以上！2023 AAO 視覺復健指引將「藥物安全管理」列為復健關鍵介入領域。本文深入...",
    "imageUrl": "https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健",
      "認知復健"
    ],
    "dateString": "2026-04-23",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/014_隱形用藥風暴之防線_視力障礙患者安全給藥管理、智慧藥盒與語音標籤防錯機制.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "環境人因工程、安全自理與防跌防護",
    "subCluster": "環境改造與居家自理",
    "order": 14,
    "folder": "VisualTherapy",
    "filename": "014_隱形用藥風暴之防線_視力障礙患者安全給藥管理、智慧藥盒與語音標籤防錯機制.html"
  },
  {
    "id": "VisualTherapy_015",
    "title": "視野缺損與合法駕駛邊界：雙焦點遠用望遠鏡（Bioptic Telescopes）、視野法規與安全駕駛評估準則",
    "lead": "被告知「您不能再開車了」對長者造成的心理衝擊，往往不亞於罹患重症。駕駛執照不僅是代步工具，更是自我價值、獨立與社交參與的基石。2023 AAO 視覺復健指引深度探討了低視能駕駛的法規邊界、醫學評估標準與爭議技術：雙焦點遠用望遠鏡（Bioptic Telescopes, BTS）。指引強調：靜態視力表無法全盤預測駕駛安全...",
    "imageUrl": "https://images.unsplash.com/photo-1449965408869-eaa3f722e40d?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健",
      "認知復健"
    ],
    "dateString": "2026-04-29",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/015_視野缺損與合法駕駛邊界_雙焦點遠用望遠鏡、視野法規與安全駕駛評估準則.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "特殊病徵、心理調適與跨專業協同",
    "subCluster": "跨專業全人照護",
    "order": 15,
    "folder": "VisualTherapy",
    "filename": "015_視野缺損與合法駕駛邊界_雙焦點遠用望遠鏡、視野法規與安全駕駛評估準則.html"
  },
  {
    "id": "VisualTherapy_016",
    "title": "大腦幻影之去病理化：夏爾·博內症候群（Charles Bonnet Syndrome）神經脫抑制機制、患者衛教與安撫對策",
    "lead": "眼前突然看見身穿十九世紀華麗宮廷服飾的小人走動、客廳地板開滿豔麗玫瑰、或是牆壁佈滿不斷旋轉的精緻幾何萬花筒——許多低視能長者飽受生動逼真的「視幻覺」折磨，卻因極度恐懼被家人認定為「發瘋、中邪或失智症」而默默隱忍，陷入巨大心理崩潰。2023 AAO 視覺復健指引明確指出：夏爾·博內症候群（Charles Bonnet S...",
    "imageUrl": "https://images.unsplash.com/photo-1559757175-5700dde675bc?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健",
      "認知復健"
    ],
    "dateString": "2026-05-08",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/016_大腦幻影之去病理化_夏爾博內症候群神經脫抑制機制、患者衛教與安撫對策.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "特殊病徵、心理調適與跨專業協同",
    "subCluster": "跨專業全人照護",
    "order": 16,
    "folder": "VisualTherapy",
    "filename": "016_大腦幻影之去病理化_夏爾博內症候群神經脫抑制機制、患者衛教與安撫對策.html"
  },
  {
    "id": "VisualTherapy_017",
    "title": "撫平失明焦慮與憂鬱幽谷：低視能患者心理健康縱向篩檢、支持團體與賦能諮商介入",
    "lead": "失去了清晰的雙眼，世界彷彿瞬間安靜並關上了大門。醫學文獻證實，視力喪失引發的心理創傷，在強度上等同於失去至親或身患惡性腫瘤。低視能族群中重度憂鬱症與焦慮症的盛行率超過 30% 至 35%，然而常規眼科門診常只專注於眼底 OCT 圖像，使得心理崩潰淪為「隱形死角」。2023 AAO 視覺復健指引將心理健康縱向篩檢（PHQ...",
    "imageUrl": "https://images.unsplash.com/photo-1527137342181-19aab11a8ee8?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健",
      "認知復健"
    ],
    "dateString": "2026-05-17",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/017_撫平失明焦慮與憂鬱幽谷_低視能患者心理健康縱向篩檢、支持團體與賦能諮商介入.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "特殊病徵、心理調適與跨專業協同",
    "subCluster": "跨專業全人照護",
    "order": 17,
    "folder": "VisualTherapy",
    "filename": "017_撫平失明焦慮與憂鬱幽谷_低視能患者心理健康縱向篩檢、支持團體與賦能諮商介入.html"
  },
  {
    "id": "VisualTherapy_018",
    "title": "破除「無能為力」的醫病溝通：壞消息傳達策略（SPIKES）、共感引導與功能賦能臨床對話",
    "lead": "「您的視網膜已經萎縮疤痕化了，醫學上我們已經沒有別的辦法了，您回去吧。」在眼科門診中，這句無心之言如同一記重錘，徹底擊碎了無數患者活下去的勇氣。2023 AAO 視覺復健指引直面這項長期存在的臨床溝通弊病，發出震撼宣告：醫療技術無法逆轉病理組織，絕不等於醫療團隊「無能為力」！指引正式將「敏感同理溝通（Sensitive...",
    "imageUrl": "https://images.unsplash.com/photo-1559839734-2b71ea197ec2?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健"
    ],
    "dateString": "2026-05-23",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/018_破除無能為力的醫病溝通_壞消息傳達策略SPIKES、共感引導與功能賦能臨床對話.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "特殊病徵、心理調適與跨專業協同",
    "subCluster": "跨專業全人照護",
    "order": 18,
    "folder": "VisualTherapy",
    "filename": "018_破除無能為力的醫病溝通_壞消息傳達策略SPIKES、共感引導與功能賦能臨床對話.html"
  },
  {
    "id": "VisualTherapy_019",
    "title": "跨專業團隊協同作戰：眼科醫師、驗光師、職能治療師（OT）與定向行動師（COMS）角色分工與轉銜體系",
    "lead": "任何試圖由眼科醫師或單一專業獨自包攬所有低視能照護的嘗試，注定將以醫療倦怠與患者失能收場。2023 AAO 視覺復健指引（特別在 Appendix 6 中）以宏大的跨學科視野，明確界定了跨專業團隊（Interprofessional Team）的四大核心支柱：眼科專科醫師、低視能驗光師、職能治療師（OT）與定向行動專家...",
    "imageUrl": "https://images.unsplash.com/photo-1582750433449-648ed127bb54?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健",
      "中風復健"
    ],
    "dateString": "2026-06-01",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/019_跨專業團隊協同作戰_眼科醫師、驗光師、職能治療師OT與定向行動師COMS角色分工與轉銜體系.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "特殊病徵、心理調適與跨專業協同",
    "subCluster": "跨專業全人照護",
    "order": 19,
    "folder": "VisualTherapy",
    "filename": "019_跨專業團隊協同作戰_眼科醫師、驗光師、職能治療師OT與定向行動師COMS角色分工與轉銜體系.html"
  },
  {
    "id": "VisualTherapy_020",
    "title": "實證醫學的高峰檢驗：LOVIT 臨床試驗、Cochrane 系統性文獻回顧與未來前沿科技展望",
    "lead": "視覺復健究竟是主觀安慰劑，還是具備嚴格實證支持的現代醫學介入？2023 AAO 視覺復健指引在 Appendix 4 中進行了系統性實證文獻回顧，深入剖析了美國退伍軍人局里程碑式的 LOVIT 與 LOVIT II 隨機對照試驗，以及 Cochrane 系統性文獻回顧。數據無可辯駁地證實：高強度跨專業視覺復健能帶來閱讀...",
    "imageUrl": "https://images.unsplash.com/photo-1527613426441-4da17471b66d?auto=format&fit=crop&w=1200&q=80",
    "tags": [
      "視覺復健"
    ],
    "dateString": "2026-06-10",
    "readTime": "約 5 分鐘閱讀",
    "link": "/content/VisualTherapy/020_實證醫學的高峰檢驗_LOVIT臨床試驗、Cochrane系統性文獻回顧與未來前沿科技展望.html",
    "sourceName": "視覺復健",
    "category": "視覺復健",
    "cluster": "特殊病徵、心理調適與跨專業協同",
    "subCluster": "跨專業全人照護",
    "order": 20,
    "folder": "VisualTherapy",
    "filename": "020_實證醫學的高峰檢驗_LOVIT臨床試驗、Cochrane系統性文獻回顧與未來前沿科技展望.html"
  }
];
