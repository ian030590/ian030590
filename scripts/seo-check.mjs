...
function auditPage(rawHtml, page) {
  ...
  if (articleish) {
    ...
    /* 直接引言：blockquote／q 標籤，或引號包住的較長句子。
       短引號多半是強調用法而不是引言，所以要求一定長度——
       **而那個長度必須分中西兩套**，理由與 title/description 的門檻同源
       （見 limitsFor）：中文一字一義，8 個字已經是一個短句；
       8 個拉丁字元只有一兩個單字，套同一個數字會把 "the fund"、"risk-free"
       這種強調用法全算成引言。

       ⚠ 這是實測出來的盲區，值得記著判斷方式。
       同一門課的投資週次頁：中文版引言 18–27 處，英文版 0 處——
       翻譯時把「」換成 "…"，整批訊號就消失了。26 頁英文課程頁裡 24 頁
       因此被誤報成「缺直接引言」，而它們其實各有 21–23 處引言。
       **同一份內容的兩個語言版本結果差這麼多，該先懷疑量測而不是內容。**

       拉丁門檻取 40：中文 8 字約等於一個短句；英文一個單字連空白約 6 字元，
       八個詞約 48，取 40 略寬一些。實測在這批語料上 25 與 40 給出相同結果
       （同樣救回 36 頁），代表結論對這個數字不敏感——所以不必為它爭論，
       但也因此不宜宣稱它精確。

       單引號不算：英文的撇號（don't、isn't）會兩兩配對成假引言。 */
    const quotes =
