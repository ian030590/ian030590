...
- **審慎保守臨床用詞準則（Conservative Phrasing Principle）**：除非指引或文獻證據非常確切（如列為 Harm 級傷害之明確醫療禁忌），否則文章中嚴禁任意使用「禁止」、「禁用」等非保守斷言詞彙，一律使用「建議避免」、「不建議」或「建議優先考慮」等客觀審慎之保守臨床用語。
- **圖片容器防溢出與橫幅裁切規範（Responsive Images & Hero Cropping）**：所有文章圖片必須使用標準元件結構與自適應 class，嚴禁寫死未受限制之寬度以防破圖溢出容器；最新專題卡片圖檔必須帶有具體橫幅裁切參數（如 `&w=1200&h=800`），並在 CSS 上完全解耦高度，杜絕直式照片上下推寬卡片。
